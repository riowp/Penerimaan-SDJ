        ...
        executor.execute(() -> {
            try {
                ...
                for (TicketScan scan : pending) {
                    ...
                    // Fix 2: Cek duplikat di Sheets SEBELUM push
                    // Ini menangkap kasus: HP lain sudah push tiket yang sama
                    String checkResp = request(SCRIPT_URL
                        + "?action=checkTicket&ticketNumber=" + enc(scan.getTicketNumber()));
                    boolean alreadyInSheets = checkResp != null
                        && checkResp.contains("\"exists\":true");

                    if (alreadyInSheets) {
                        // Tiket sudah ada di Sheets (dari HP lain) — rollback lokal
                        Log.w(TAG, "PUSH skip+rollback duplikat: " + scan.getTicketNumber());
                        db.ticketScanDao().delete(scan);
                        // Kirim notifikasi ke operator
                        mainHandler.post(() ->
                            com.ticketscanner.app.notifications.NotificationHelper
                                .notifyDuplicateTicketRolledBack(context, scan.getTicketNumber()));
                        continue; // skip push untuk tiket ini
                    }
                    ...
                    if (resp != null && resp.contains("\"ok\"")) {
                        ...
                        // Apps Script menolak karena duplikat (Fix 3 di server)
